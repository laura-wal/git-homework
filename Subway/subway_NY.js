...
//var start = prompt('Where do you want to leave from?');
 $("#journStart").on('click', function(){
       startData = $('#start-station').val();
      console.log(startData);
 })

// Afind out where we are going
//var end = prompt('Where do you want to go?');
$("#journEnd").on('click', function(){
       endData = $('#end-station').val();
      console.log(endData);
})

// $("button").on('click', function(){
//        var StartCal = var journeyCalc;
//       console.log(StartData);
// })
...
// var distance;
 $("#journCalc").on('click', function(){
    for(var i = 0; i < allLines.length; i++){
        if ( allLines[i].indexOf(startData) >= 0 && allLines[i].indexOf(endData) >= 0 ){
            var distance = Math.abs( allLines[i].indexOf(startData) - allLines[i].indexOf(endData) );
        }    
    }
    ...
    if(!distance){
        ...
        for(var i = 0; i < allLines.length; i++){
            ...
            if(allLines[i].indexOf(startData) >= 0){
                startIndex = allLines[i].indexOf(startData);
            ...
            if(allLines[i].indexOf(endData) >= 0){
